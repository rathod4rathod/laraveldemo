...
class PixelTrackerController extends Controller
{
    ...
    public function add_pixel_tracker( Request $request )
    {
        ...
        $pixel_tracker_created = PixelTracker::create( [
                    ...
                    'redirect_url' => '',
                    'pixel_email' =>'',
                    'pixel_subject' => $filter[ 'pixel_tracker_title' ],
                ] );
        if ( $pixel_tracker_created )
        {
            ...
            return response()->json( ['flag' => 1, 'message' => 'Email Tracker added successfully!' ] );
        }
        else
        {
    ...
    public function edit_pixel_tracker( Request $request )
    {
        ...
        if ( $pixel_tracker_found )
        {
            ...
            else
            {
                ...
                return response()->json( ['flag' => 1, 'message' => 'Email Tracker updated successfully!' ] );
            }
        }
        else
        {
        ...
        if ( $status == 'activate' || $status == 'deactivate' )
        {
            ...
            if ( $pixel_tracker_found )
            {
                ...
                else
                {
                    ...
                    {
                        return response()->json( ['flag' => 0, 'message' => 'This Email Tracker is already ' . $status . 'd!' ] );
                    }
                    else
                    {
                        $pixel_tracker_found->is_active = $changed_status;
                        $pixel_tracker_found->save();
                        return response()->json( ['flag' => 1, 'message' => 'Email Tracker ' . $status . 'd successfully!' ] );
                    }
                }
            }
            else
